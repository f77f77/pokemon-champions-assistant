 ...
 * 再均分成 6 格；每格內再裁縮圖：水平 5%–38%、上下各內縮 12%。
 *
...
/**
 * 單格內縮圖裁切（相對 slot：水平 5%–38%，上下各內縮 12%）。
 * 回傳整幀像素座標。
 */
export function thumbRectInSlot(slot: PixelRect): PixelRect {
  const insetY = slot.height * THUMB_CROP.verticalInset;
  const x = slot.x + slot.width * THUMB_CROP.left;
  const y = slot.y + insetY;
  const width = slot.width * (THUMB_CROP.right - THUMB_CROP.left);
  const height = slot.height * (1 - 2 * THUMB_CROP.verticalInset);
  return {
    x: Math.floor(x),
    y: Math.floor(y),
    width: Math.max(1, Math.floor(width)),
...
export function thumbCssPercent(
  ...
  const top = slotTop + slotH * THUMB_CROP.verticalInset;
  const width = panelW * (THUMB_CROP.right - THUMB_CROP.left);
  const height = slotH * (1 - 2 * THUMB_CROP.verticalInset);
  return {
    left: left * 100,
    top: top * 100,
    width: width * 100,
